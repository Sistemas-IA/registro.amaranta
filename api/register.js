export default async function handler(req, res) {
  ...
  const { nombre, apellido, dni, telefono, email, direccion, comentarios, zona, estado } =
    Object.fromEntries(await req.body instanceof FormData ? req.body.entries() : []);

  const recaptchaToken = req.body.get?.("g-recaptcha-response") || req.body["g-recaptcha-response"];
  ...
  const payload = {
    ...
    zona: "Pendiente", estado: "Pendiente", timestamp: new Date().toISOString(), ip
  };
